use crate::{sp_runtime::RuntimeDebug, Config, MoveGlobalStorage, LOG_TARGET};
...
#[derive(Hash, Encode, Decode, Clone, PartialEq, Eq, RuntimeDebug, TypeInfo, MaxEncodedLen)]
...
pub fn store<T: Config>(address: [u8; 32], tag: [u8; 32], guest_data: alloc::vec::Vec<u8>) {
	let bounded_data = BoundedVec::<u8, ConstU32<2048>>::try_from(guest_data.clone()).expect("failed to convert to BoundedVec");
...
pub fn update<T: Config>(address: [u8; 32], tag: [u8; 32], new_data: alloc::vec::Vec<u8>) {
	...
	MoveGlobalStorage::<T>::mutate(&move_address, &struct_tag_hash, |maybe_entry| {
		if let Some(entry) = maybe_entry {
			if entry.borrow_mut {
				let bounded_data =
					BoundedVec::<u8, ConstU32<2048>>::try_from(new_data.clone()).expect("failed to convert to BoundedVec");
